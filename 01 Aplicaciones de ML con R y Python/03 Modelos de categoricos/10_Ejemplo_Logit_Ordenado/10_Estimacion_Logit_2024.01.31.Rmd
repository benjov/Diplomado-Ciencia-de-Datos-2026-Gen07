...
# Import the data (se carga desde el ZIP local para mantener el repo ligero)
Datos <- read.csv(unz('Datos_SEL.csv.zip', 'Datos_SEL.csv'))
#dataframe %>% head()
dim(Datos)
...
1. En la sección "Select subset: 1. Menos de 1M ~ 10%", se toma solo un 10% aleatorio de los
   juegos con ingresos "Menos de 1M" (la clase mayoritaria) antes de combinarlos con el resto y
   entrenar el modelo. ¿Qué hubiera pasado con `LOGIT_01` si se hubiera entrenado con la base
   completa, sin balancear las clases? (Pista: piensa en qué predicción trivial —"siempre digo
   la clase mayoritaria"— ya tendría una exactitud alta con clases desbalanceadas, y qué tan útil
   sería un modelo así en la práctica).

2. El modelo se evalúa dos veces: una vez sobre `Valid` (30% de la base balanceada que nunca vio
   el entrenamiento) y otra vez sobre `Datos` (la base original completa, con el desbalance de
   clases real). Compara las dos matrices de confusión. ¿El desempeño es parecido? Si cambia,
   ¿a qué se debe — el modelo "olvidó" lo aprendido, o es un efecto del cambio en la composición
   de clases entre `Valid` y `Datos`?

3. Revisa la salida de `LM_01_ME` (efectos marginales). Elige dos tags con signo opuesto (uno que
   aumente y otro que reduzca la probabilidad de superar 1M en ingresos) e interpreta qué
   significa ese signo en términos del negocio: ¿tiene sentido que ese tag esté asociado con
   mayores o menores ingresos?

4. **Ejercicio:** Modifica una variable de `setup_01` (por ejemplo, cambia `Price` o agrega/quita
   un tag como `Multiplayer`) y vuelve a correr la predicción. ¿Qué tanto cambia la probabilidad
   estimada? ¿El cambio es consistente con el signo del efecto marginal de esa variable en
   `LM_01_ME`?