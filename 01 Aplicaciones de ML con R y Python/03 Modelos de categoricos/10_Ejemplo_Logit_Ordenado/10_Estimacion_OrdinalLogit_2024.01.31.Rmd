...
1. A diferencia de `10_Estimacion_Logit_2024.01.31.Rmd` (2 categorías), aquí `Cat.Rev.Year` tiene
   4 niveles ordenados. Observa la matriz de confusión: ¿los errores del modelo tienden a
   "confundir" categorías adyacentes (por ejemplo, predecir "Más de 1M y menos de 5M" cuando lo
   observado es "Menos de 1M") o categorías muy alejadas entre sí (confundir el nivel más bajo
   con el más alto)? ¿Qué te dice eso sobre qué tan bien el modelo captura el orden de la
   variable?

2. Compara los coeficientes de `OLM_01` (logit ordinal) con los de `LOGIT_01` del ejercicio
   binario, para las variables que aparecen en ambos modelos (por ejemplo `Price`, `Score`,
   `Multiplayer`). ¿El signo es consistente entre ambos modelos? El logit ordinal asume que el
   efecto de cada variable es el mismo en todos los "cortes" entre categorías (odds
   proporcionales) — ¿te parece un supuesto razonable para variables como `Price`, donde el
   salto de "barato" a "medio" podría tener un efecto distinto al de "medio" a "caro"?

3. **Ejercicio:** Al igual que en el ejemplo binario, cambia una variable de `setup_01` (por
   ejemplo `Multiplayer` o `RPG`) y vuelve a correr `predict(OLM_01, newdata = setup_01, type =
   "probs")`. ¿Qué categoría gana probabilidad y cuál la pierde? ¿El cambio es consistente con lo
   que reportan los efectos marginales de `OLM_01_ME`?