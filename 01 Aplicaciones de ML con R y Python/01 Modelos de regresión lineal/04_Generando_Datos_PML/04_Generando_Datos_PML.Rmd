...
#install.packages("zip") # Para comprimir el CSV de salida sin depender de un binario externo
...
Los datos se guardan directamente comprimidos (zip con un solo miembro, mismo nombre base que el
CSV) en vez de dejar el CSV plano en disco — mismo patrón que usan los notebooks que después leen
estos archivos (`Clustering_Time_Series_PML.ipynb`), vía `read.csv(unz('archivo.csv.zip',
'archivo.csv'))`.

```{r save}
guardar_zip <- function(df, nombre_csv) {
  write.csv(df, nombre_csv, row.names = FALSE)
  zip::zip(paste0(nombre_csv, ".zip"), nombre_csv)
  file.remove(nombre_csv)
}

guardar_zip(Datos_PML, "Datos_PML.csv")
guardar_zip(Datos_PML_Wide, "Datos_PML_Wide.csv")
